<template>
  <div @click="toggleFavorite" class="cursor-pointer">
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      <path
        :fill="isFavorited ? '#ED3F3F' : 'white'"
        :stroke="isFavorited ? 'none' : 'gray'"
        stroke-width="2"
...
export default {
  ...
  data() {
    ...
    return {
      isFavorited: favoritesStore.isFavorite(this.productId),
    };
  },
  methods: {
    toggleFavorite() {
      if (this.isFavorited) {
        ...
        this.favoritesStore.removeFavorite(this.productId);
      } else {
        this.isFavorited = true;
        this.favoritesStore.addFavorite(this.productId);
      }
    },
  },
  created() {
